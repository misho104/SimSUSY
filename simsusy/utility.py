"""Utility functions common for any models."""
...
def is_tiny(x: float, delta: float = 1e-6) -> bool:
    """Return if x is close to zero."""
    return -delta < x < delta


def sin2cos(sin: float) -> float:
    """Return Cos[ArcSin[x]] assuming -pi/2 < angle < pi/2."""
    # result in [0, 1]
...
def cos2tan(cos: float) -> float:
    """Return Tan[ArcCos[x]] assuming 0 < angle < pi."""
    # result in [0, inf]
    return math.sqrt((1 - cos) * (1 + cos)) / cos


def tan2costwo(tan: float) -> float:
    """Return Cos[2*ArcTan[x]] assuming -pi/2 < angle < pi/2."""
    # result in [-1, 1]
    return (1 + tan) * (1 - tan) / (tan * tan + 1)


def tan2sintwo(tan: float) -> float:
    """Return Sin[2*ArcTan[x]] assuming -pi/2 < angle < pi/2."""
    # result in [-1, 1]
    return 2 * tan / (tan * tan + 1)


def tan2tantwo(tan: float) -> float:
    """Return Tan[2*ArcTan[x]] assuming -pi/2 < angle < pi/2."""
    # result in [-inf, inf]
    return 2 * tan / (1 + tan) / (1 - tan)


def chop_matrix(m, threshold=1e-15):
...
def is_diagonal_matrix(m: Any) -> bool:
    """Return if m is 2d NDArray with all off-diagonal entries being zero."""
    if not (isinstance(m, np.ndarray) and m.ndim == 2):
        return False
...
def autonne_takagi(m, try_real_mixing=True):
    ...
    """
    eigenvalues, eigenvectors = numpy.linalg.eigh(np.conjugate(m) @ m)
...
def singular_value_decomposition(m: Matrix) -> Tuple[RealMatrix, Matrix, Matrix]:
    ...
    """
    u0, s, vh0 = numpy.linalg.svd(m)  # u0 @ s @ vh0 = m, i.e. u0^† @ m @ v0h^† = s
